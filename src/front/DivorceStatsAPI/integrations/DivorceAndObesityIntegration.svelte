<script>
    ...
    async function onLoad() {
        ...
        divorceStats.forEach((element) => {
            if (divorceCountries[element.country] == null) {
                divorceCountries.push(element);
            }
        }),
            obesityStats.forEach((element) => {
                if (obesityCountries[element.country] == null) {
                    obesityCountries.push(element);
                }
            }),
        ...
        divorceStats.forEach((e) => {
            ...
            if (
                divorceCountries[e.country] != null &&
                datos[{ country: e.country }] == null
            ) {
            ...
            obesityStats.forEach((e) => {
                if (
                    paisesComunes[e.country] != null &&
                    datos[{ country: e.country }] != null
                ) {
                    dato = datos[{ country: e.country }];
                    dato.man_obesity = e.man_percent;
                    dato.woman_obesity = e.woman_percent;
                    datos.update(dato);
                }
            });
        ...
        datos.forEach((e) => {
            var point1 = { x: e.country, y: e["divorce-rate"] };
