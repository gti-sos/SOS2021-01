<script>
	...
	*/
    ...
    async function getStats(country, year) {
        console.log("Fetching divorce stats...");
        ...
        if (res.ok && next.ok) {
            ...
            console.log("Received " + divorceStats.length + " divorce stats.");
        } else {
    ...
    async function loadInitialStats() {
        console.log("Loading initial divorce stats...");
        deleteStats();
    ...
    async function insertStat() {
        console.log("Inserting divorce stats...");
        /* Checking if the country and the year are not empty */
        if (
            newDivorceStat.country == "" ||
            newDivorceStat.country == null ||
            newDivorceStat.year == "" ||
            newDivorceStat.year == null
        ) {
            alert(
            ...
            const res = await fetch(BASE_API_URL, {
                method: "POST",
                body: JSON.stringify(newDivorceStat),
                headers: {
    ...
    async function deleteStats() {
        console.log("Deleting divorce stats...");
        const res = await fetch(BASE_API_URL + "/", {
            method: "DELETE",
        }).then(function (res) {
            if (res.ok) {
...
<main>
            ...
            <tbody>
                <tr>
                    <td>
                        <Input
                            ...
                            bind:value={newDivorceStat.country}
                        />
                    </td>
                    <td>
                        <Input
                            type="number"
                            placeholder="Ej. 2020"
                            bind:value={newDivorceStat.year}
                        />
                    </td>
                    <td>
                        <Input
                            ...
                            step="0.01"
                            min="0"
                            bind:value={newDivorceStat["marriage-rate"]}
                        />
                    </td>
                    <td>
                        <Input
                ...
                {#each divorceStats as stat}
                    <tr>
                        <td>
                            <a
                                href="#/divorce-stats/{stat.country}/{stat.year}"
                            >
                                {stat.country}
                            </a>
                        </td>
    ...
    <Pagination style="float:right;" ariaLabel="Cambiar de página">
        ...
        {#if currentPage != 1}
            <PaginationItem>
                <PaginationLink href="#/DivorceAPI" on:click={() => addOffset(-1)}
                    >{currentPage - 1}</PaginationLink
                >
            </PaginationItem>
        {/if}
        <PaginationItem active>
            <PaginationLink href="#/DivorceAPI">{currentPage}</PaginationLink>
        </PaginationItem>
